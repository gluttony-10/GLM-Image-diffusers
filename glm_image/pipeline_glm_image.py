...
from diffusers.utils import is_torch_xla_available, is_transformers_version, logging, replace_example_docstring
...
class GlmImagePipeline(DiffusionPipeline):
    ...
    def check_inputs(
        ...
        # Modified: Only check if prior_token_ids is None but prior_image_token_ids is not
        # Allow prior_token_ids to exist while prior_image_token_ids is None (for T2I)
        if (prior_token_ids is None and prior_image_token_ids is not None):
            raise ValueError(
